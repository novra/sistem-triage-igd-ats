export function toTrainingDataset(records: any[]) {
  ...
  return records.map((r) => ({
    ...
    predicted_ats_category: r.atsPrediction?.atsCategory || "",
    ai_recommended_ats: r.atsPrediction?.decisionSupport?.aiRecommendation?.atsLevel || "",
    guardrail_recommended_ats: r.atsPrediction?.decisionSupport?.guardRailRecommendation?.atsLevel || "",
    ai_guardrail_different: r.atsPrediction?.decisionSupport?.recommendationsDiffer ? 1 : 0,
    guardrail_applied: r.atsPrediction?.decisionSupport?.guardRailApplied ? 1 : 0,
    ...
    clinical_information_used: r.atsPrediction?.informasiKlinisDigunakan ? r.atsPrediction.informasiKlinisDigunakan.join(" | ") : "",
    additional_information_needed: r.atsPrediction?.informasiTambahanDiperlukan ? r.atsPrediction.informasiTambahanDiperlukan.join(" | ") : "",
    initial_interventions: r.atsPrediction?.rekomendasiAwal ? r.atsPrediction.rekomendasiAwal.join(" | ") : "",
